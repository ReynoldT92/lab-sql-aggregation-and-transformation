...
-- 1.3 Number of films for each rating, sorted by count desc
SELECT 
    rating,
    COUNT(*) AS films_per_rating
FROM film
...
FROM film
GROUP BY rating 
HAVING AVG(length) > 120          -- <-- this HAVING was missing
ORDER BY mean_duration_minutes DESC;
